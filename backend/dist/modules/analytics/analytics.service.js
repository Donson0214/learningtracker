...
exports.getOrgDashboard = exports.getLearnerDashboard = void 0;
...
const getOrgDashboard = async (organizationId) => {
    const [users, courses, sessions, enrollments, planItems, goals] = await Promise.all([
        ...
        prisma_1.prisma.studyGoal.findMany({
            where: { user: { organizationId } },
    ...
    enrollments.forEach((enrollment) => {
        const bucket = enrollmentBuckets.find((entry) => enrollment.createdAt >= entry.start &&
    ...
    sessions.forEach((session) => {
        ...
        const title = existing?.title ?? session.course?.title ?? "Untitled";
    ...
    planItems.forEach((item) => {
        ...
        completionMap.set(item.courseId, {
            ...
            completed: (existing?.completed ?? 0) + (item.isCompleted ? 1 : 0),
        });
    });
    const engagementWindow = new Date();
    engagementWindow.setDate(engagementWindow.getDate() - 30);
    const engagementMinutes = new Map();
    sessions.forEach((session) => {
        if (session.studiedAt < engagementWindow) {
            return;
        }
        engagementMinutes.set(session.userId, (engagementMinutes.get(session.userId) ?? 0) +
    ...
    const targetMinutesPerWeek = Array.from(latestGoals.values()).reduce((sum, hours) => sum + hours * 60, 0);
    const paceBuckets = buildWeekBuckets(4);
    sessions.forEach((session) => {
        const bucket = paceBuckets.find((entry) => session.studiedAt >= entry.start &&
    ...
    return {
        ...
        completionRates: Array.from(completionMap.entries()).map(([courseId, data]) => ({
            courseId,
            title: data.title,
