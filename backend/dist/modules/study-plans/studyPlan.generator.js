...
const generateStudyPlan = ({ lessons, startDate, endDate, minutesPerWeek, rescheduleFrom, }) => {
    const plan = [];
    if (minutesPerWeek <= 0 || startDate > endDate) {
        return plan;
    }
    const scheduleStart = rescheduleFrom && rescheduleFrom > startDate
        ? rescheduleFrom
        : startDate;
    ...
    for (const lesson of lessons) {
        ...
        while (remaining > 0) {
            if (currentDate > endDate)
                break;
            const daysFromStart = Math.floor((currentDate.getTime() - scheduleStart.getTime()) /
                (1000 * 60 * 60 * 24));
            ...
            const weekRemaining = Math.max(0, minutesPerWeek - minutesScheduledThisWeek);
            if (weekRemaining === 0) {
                currentDate.setDate(currentDate.getDate() + 1);
                continue;
            }
            const minutesForDay = Math.min(remaining, weekRemaining, dailyCap);
            plan.push({
